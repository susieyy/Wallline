...
{
    return [self stringMatchedByRegex:regex caseInsensitive:NO treatAsOneLine:NO];
}

-(NSString*) stringMatchedByRegex:(NSString *)regex caseInsensitive:(BOOL) ignoreCase treatAsOneLine:(BOOL) assumeMultiLine 
{
    NSError *error = nil;
    NSUInteger options=0;
    if (ignoreCase) {
    ...
    NSRegularExpression *regexp = [NSRegularExpression regularExpressionWithPattern:regex options:options error:&error];
    if (error != nil) {
        return nil;
